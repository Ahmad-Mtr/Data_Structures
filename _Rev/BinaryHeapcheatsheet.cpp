...
//deletetop()
    #define Global Scope:
static const int MAX_SIZE = ;   //  MaxS
int heap[MAX_SIZE];             //
int _size = 0;                  // Current size
/////////////////////////////////////////////
    #define Index Methods:
        *//All of type static int;
        *//(i - 1) / 2;  // Parent
        *//2*i + 1;      //left
        *//2*i + 2;      //Right
...
void insert(int data) {
    ...
    while (i > 0 && heap[i] > heap[parent(i)]) {    // if i is positive AND heap[i] is Bigger than its parent
        int temp = heap[i];             //  |
        heap[i] = heap[parent(i)];      //  |
        heap[parent(i)] = temp;         // Swapping code
        i = parent(i);      // let/update i be the parent
    }
}
// returns the maximum item of the heap
int getMax() {
    if (_size == 0) {   //Base case
